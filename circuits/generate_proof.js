 ...
 *
 * Usage:
 *   node generate_proof.js <actual_salary> <blinding_factor>
 *
...
// ── CLI args ──────────────────────────────────────────────────────────────────
const actualSalary = BigInt(process.argv[2] ?? "5000");
const blindingFactor = BigInt(process.argv[3] ?? "123");
...
(async () => {
	...
	if (snarkjs && hasArtifacts) {
		await generateRealProof(snarkjs, actualSalary, blindingFactor);
	} else {
		if (!snarkjs) {
		...
		generateMockProof(actualSalary, blindingFactor);
	}
})().catch((err) => {
...
async function generateRealProof(snarkjs, actualSalary, blindingFactor) {
	const input = {
		actual_salary: actualSalary.toString(),
...
function generateMockProof(actualSalary, blindingFactor) {
	const s = actualSalary % BN254_R;
	const b = blindingFactor % BN254_R;

	// Deterministic mock values for the two public signals in payment.circom.
	const onChainCommitment = (s + b * 7n) % BN254_R;
	const expectedTransferAmount = s;
	...
	const c_x = (onChainCommitment * 7n + 3n) % BN254_R;
	const c_y = (expectedTransferAmount * 11n + 4n) % BN254_R;
...
function writeProofBytesFile(proof, publicSignals) {
	...
	const on_chain_commitment = fe(publicSignals[0]);
	const expected_transfer_amount = fe(publicSignals[1]);
	...
	const proofBytes = {
		...
		// Backward-compat aliases used by existing Rust helper/tests.
		salary_commitment: on_chain_commitment,
		payment_nullifier: expected_transfer_amount,
		recipient_hash: expected_transfer_amount,
	};
